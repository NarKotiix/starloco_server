...
public class Formulas {
    ...
    public static ArrayList<Integer> chanceFM(int WeightTotalBase,
        ...
        if (c < 0)
            c = 0;
        // la variable c reste à 1 si le jet ne depasse pas 80% sinon il diminue trÃ¨s fortement. Si le jet dépasse 100% alors il diminue encore plus.
    ...
    private static final Pattern LOWER_ACCENTS = Pattern.compile("[a-zàâäéèêëïîôöùûüÿçñ]");
    private static final Pattern UPPER_ACCENTS = Pattern.compile("[A-ZÀÂÄÉÈÊËÏÎÔÖÙÛÜŸÇÑ]");

    public static String translateMsg(String msg) {
        msg = LOWER_ACCENTS.matcher(msg).replaceAll("m");
        msg = UPPER_ACCENTS.matcher(msg).replaceAll("H");
        return msg;
    }
