...
public class House {
    ...
    public static void buy(Player player)//Acheter une maison
    {
        ...
        for (Trunk trunk : Trunk.getTrunksByHouse(house)) {
            if (house.getOwnerId() > 0)
                //Déplacement des items vers la banque
                trunk.moveTrunkToBank(World.world.getAccount(house.getOwnerId()));
    ...
    public void enter(Player P) {//Entrer dans la maison
        if (P.getFight() != null || P.getExchangeAction() != null)
            return;
        if (this.getOwnerId() == P.getAccID() || (P.get_guild() != null && P.get_guild().getId() == this.getGuildId() && canDo(Constant.H_GNOCODE)))//C'est sa maison ou mÃªme guilde + droits entrer sans pass
            open(P, "-", true);
    ...
    public void parseIntToRight(int total) {
        ...
        if (haveRight.size() > 0) //Si les droits contiennent quelque chose -> Vidage (MÃªme si le HashMap supprimerais les entrées doublon lors de l'ajout)
            haveRight.clear();
