        ...
        try {
            ...
            if (split != null && split.length > 0) {
                for (String loc1 : split) {
                    ...
                    // answer.getActions() avec id = 40, on récupère l'id de quÃªte. Si on a la quÃªte alors on affiche pas !
                    if (answer == null)
                        continue;
                    boolean ok = true;
                    for (Action action : answer.getActions()) // par action
                    {
                        switch (action.getId()) {
                            ...
                            case 40: // Si on apprend une quÃªte
                                if (!player.getQuestPerso().isEmpty()) {
                                    for (QuestPlayer QP : player.getQuestPerso().values()) {
                                        if (QP.getQuest().getId() == Integer.parseInt(action.getArgs()))
                                            ok = false; // S'il a la quÃªte on dégage
                                    }
                                }
                                break;
                    ...
                    if (!player.getQuestPerso().isEmpty() && ok) // par quÃªte
                    {
                        for (QuestPlayer QP : player.getQuestPerso().values()) {
                    ...
                    if (ok) {
                        ...
                        for (int v = 0; v < s.length; v++) {
                            if (Integer.parseInt(s[v][6]) == answer.getId()) // Si la réponse est une traque de monstres
                            {
                                for (QuestPlayer QP : player.getQuestPerso().values()) {
                                    boolean k = true;
                                    if (QP.getQuest().getId() == Integer.parseInt(s[v][5])) // S'il a la quÃªte
                                    {
                                        k = false;
