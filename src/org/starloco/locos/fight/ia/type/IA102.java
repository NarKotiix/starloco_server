	  ...
	  if(!this.stop&&this.fighter.canPlay()&&this.count>0)
	    {
	      ...
	      // Détecte allier é PO max +1 +3 cases (Sur officiel se déplace de 3 cases pour booster son invocateur)
	      if(C!=null&&C.isHide())
	        C=null;
	      ...
	      if(this.fighter.getCurPm(this.fight)>0&&C==null&&this.attack==0)
	      {
	    	...
	    	// On essaie d'aller en face pour fléche ralentissante
	      ...
	      if(this.fighter.getCurPa(this.fight)>0&&C!=null&&!action)
	      {
	    	 ...
	    	 // int celltf=fighter.getCell().getId();
	          ...
	          // On évite de se déplacer inutilement en ligne si on y est déjé
	          if(isEnFace==0)
	          {
